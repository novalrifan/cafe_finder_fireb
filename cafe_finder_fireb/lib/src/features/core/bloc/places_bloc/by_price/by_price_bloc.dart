...
part 'by_price_event.dart';
part 'by_price_state.dart';

class ByPriceBloc extends Bloc<ByPriceEvent, ByPriceState> {
  // ignore: unused_field
  ...
  ByPriceBloc({
    required CoffeeRepo coffeeRepo,
        ...
        super(ByPriceLoading()) {
    on<LoadByPrice>(_onLoadByPrice);

    // on<UpdateLocation>(_onLoadDistances);

    _coffeeSubsription = _coffeeRepo.getCoffeeshops().listen(
          (coffeeshops) => add(LoadByPrice(coffeeshops)),
        );
  }
  void _onLoadByPrice(
    LoadByPrice event,
    Emitter<ByPriceState> emit,
  ) {
    emit(ByPriceLoaded(event.coffeeshops));
  }
